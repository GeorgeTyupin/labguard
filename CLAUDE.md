...
- `/products` — список доступных для покупки продуктов с inline-кнопками
- `/my` — мои покупки и токен
- `/devices` — сброс fingerprint

**Флоу покупки (текущая реализация):**
1. Пользователь: `/products`
2. Бот: показывает список некупленных продуктов с inline-кнопками
3. Пользователь: нажимает на продукт
4. Бот: показывает детали продукта и кнопку "Купить 🛒"
5. Пользователь: нажимает "Купить 🛒"
6. Бот: создаёт purchase через API (пока без платёжной системы)

**Флоу покупки (планируется):**
1. Пользователь: выбирает продукт через `/products`
